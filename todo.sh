...
# Check terminal size
if [ "$(tput lines)" -lt 30 ] || [ "$(tput cols)" -lt 100 ]; then
  echo "Please enlarge your terminal window (min 100 cols x 30 lines)."
  exit 1
fi
...
[ -f "$STREAK_FILE" ] || echo '{"current_streak": 0, "stars_this_week": 0, "last_updated_date": ""}' > "$STREAK_FILE"
...
streak_window() {
    ...
    gum style --border "$BORDER_STYLE" --border-foreground "$COLOR_ACCENT" --padding "$PADDING" --width 20 \
        "$(printf "🔥 Streak: %d\n%s\n\n%s/7" "$current_streak" "$star_display" "$stars")"
}
...
update_streak() {
    ...
    local total_daily_tasks=$(jq 'length' "$DAY_TASK_FILE")
    local completed_tasks=$(jq '[.[] | select(.completed == true)] | length' "$DAY_TASK_FILE")
    
    jq --arg date "$current_date" --arg last "$last_updated" \
        --argjson total "$total_daily_tasks" \
        --argjson completed "$completed_tasks" \
    '
    if ($total > 0 and $completed < $total) then
        # Reset if any incomplete tasks when there are tasks
        .current_streak = 0 |
        .stars_this_week = 0 |
        .last_updated_date = $date
    else
        if ($total > 0 and $last != $date) then
            # Only update once per day when all tasks are completed
            .current_streak += 1 |
            .stars_this_week = ((.stars_this_week + 1) % 7) |
            .last_updated_date = $date
        else
            .
        end
...
notes_window() {
  ...
  printf "📝 Notes\n%s" "${notes:-No notes}" | gum style --border "$BORDER_STYLE" --border-foreground "$COLOR_SECONDARY" --padding "$PADDING" --width 50
}
...
add_task_dialog() {
  ...
  for task in "${tasks[@]}"; do
    ...
    gum confirm "Add to $period: '$task'?" && {
      ...
      gum style --foreground "$COLOR_ACCENT" "✅ Added: $task"
    }
  done
}
...
remove_task_dialog() {
  ...
  mapfile -t task_list < <(jq -r '.[] | select(.completed == false) | .task' "$file" | nl -w1 -s'. ')
  [ ${#task_list[@]} -eq 0 ] && { gum confirm "No tasks in $period period." --affirmative "OK"; return; }
  local selected=$(gum choose --header "Select task to remove" "${task_list[@]}")
  [ -z "$selected" ] && return
  local task_num=$(echo "$selected" | cut -d'.' -f1)
  local task_index=$((task_num - 1))
  gum confirm "Remove task: $(echo "$selected" | cut -d'.' -f2- | xargs)?" && {
    jq --argjson idx "$task_index" 'del(.[$idx])' "$file" > tmp && mv tmp "$file"
    gum style --foreground "$COLOR_ACCENT" "Task removed from $period."
  }
}
...
complete_task_dialog() {
  ...
  mapfile -t task_list < <(jq -r '.[] | select(.completed == false) | .task' "$file" | nl -w1 -s'. ')
  [ ${#task_list[@]} -eq 0 ] && { gum confirm "No incomplete tasks in $period period." --affirmative "OK"; return; }
  local selected=$(gum choose --header "Select task to complete" "${task_list[@]}")
  [ -z "$selected" ] && return
  local task_num=$(echo "$selected" | cut -d'.' -f1)
  local task_index=$((task_num - 1))
  gum confirm "Complete task: $(echo "$selected" | cut -d'.' -f2- | xargs)?" && {
    jq --argjson idx "$task_index" '.[$idx].completed = true' "$file" > tmp && mv tmp "$file"
    update_stars "$period"
    
    local current_earned=$(cat "$STARS_EARNED_FILE")
    echo $((current_earned + 1)) > "$STARS_EARNED_FILE"
